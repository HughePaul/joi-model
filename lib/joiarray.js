...
var JoiArray = function(schema, options, data) {
    ...
    var rules = schema._inner.inclusions;
    var objectTypes = [];
    var otherTypes = [];
    for (var ri = 0; ri < rules.length; ri++) {
        var rule = rules[ri];
        if(rule._type === 'object') {
            objectTypes.push(rule);
        } else {
            otherTypes.push(rule);
        }
    }
...
JoiArray.prototype.validate = function(data) {
    var result = Joi.validate(data || this, this._schema, this._options);
    if (result.error) {
        throw result.error;
    }
};
...
// override splice
JoiArray.prototype.splice = function(index, length) {
    var items = Array.prototype.slice.call(arguments, 2);
    items = [index, length].concat(this.applyModel(items));
    var newArray = Array.prototype.splice.call(this);
    Array.prototype.splice.apply(newArray, items);
    this.validate(newArray);
    return Array.prototype.splice.apply(this, items);
};
